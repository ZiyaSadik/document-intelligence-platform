...
| Public repository | `<FILL IN AFTER DEPLOY>` |
...
**Tax-inclusive receipts are handled explicitly.** When the document states the
total already contains tax (`tax_inclusive: true` — "GST inclusive" and
similar), adding the tax again would fail spuriously, so the check becomes
`subtotal − discount (+ rounding) = total` and says so in its `formula` field.
...
running the actual code, never hand-written (`scripts/generate_samples.py`).
Covers the brief's test matrix: all four document types, a scanned image, a
native-text PDF and a vector-drawn one, a multi-page statement, a
validation-failure case, and a rejected unsupported file.

```bash
python scripts/generate_samples.py            # everything
python scripts/generate_samples.py --only invoice
```
...
2. **The concept registry is English-language and oriented to the statement
...
3. **Processing is synchronous** and takes roughly 15–40 s per page for
   rasterised statements. The brief permits this; it would not survive real
   concurrency.
4. **The section-sum check needs unambiguous structure** — exactly one total row
...
5. **Three pages maximum**, per the brief. Nothing in the design prevents more;
